...
behind every file handle, lock, and timer in production C++. Every snippet's output was
verified with `clang++ -std=c++20 -Wall -Wextra -Werror=return-type`.

Every term is defined the first time it appears; every snippet's output, error message,
and exit code was verified with
`clang++ -std=c++20 -Wall -Wextra -Werror=return-type` on this Mac.
