...
public:
    ...
    double read() override { return 30.0; }
};

class Lidar : public Sensor {
public:
    ~Lidar() override { destructionLog().push_back("Lidar"); }
    std::string name() const override { return "lidar"; }
    double read() override { return 10.0; }
};
...
// does not override it. There is no vtable here at all.
struct BrokenBase {
...
// Build a BrokenDerived, view it through a BrokenBase*, return p->id().
// Without virtual, the POINTER's type picks the function at compile time:
// the base version runs even though the object is a BrokenDerived.
std::string brokenDispatchDemo() {
    BrokenDerived d;
    const BrokenBase* p = &d;
    return p->id();  // -> "BrokenBase"
}
