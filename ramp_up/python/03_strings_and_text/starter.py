...
def clean_palindrome(s: str) -> bool:
    ...
    HINT: build the cleaned list with a comprehension
...
def caesar_shift(s: str, k: int) -> str:
    ...
    HINT: ord(c) / chr(n) convert between character and code point;
...
def format_report(name: str, score: int, total: int) -> str:
    ...
    HINT: one f-string with inline format specs:
...
def join_nonempty(parts: list[str], sep: str) -> str:
    ...
    HINT: sep.join(p for p in parts if p) — join takes any iterable, and
