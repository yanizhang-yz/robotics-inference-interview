...
def find_compiler() -> str:
    ...
    if compiler is None:
        pytest.skip("C++ compiler required for Quest 09")
    return compiler
...
def test_binary(tmp_path: Path) -> Path:
    ...
    binary = tmp_path / "thread_pool_tests"
    command = [
        find_compiler(),
        "-std=c++20",
        "-pthread",
...
def test_returns_future_results(test_binary: Path) -> None:
    run_case(test_binary, "future")


def test_runs_tasks_concurrently(test_binary: Path) -> None:
    run_case(test_binary, "concurrent")


def test_shutdown_rejects_new_tasks(test_binary: Path) -> None:
