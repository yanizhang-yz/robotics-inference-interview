...
def find_compiler() -> str:
    ...
    if compiler is None:
        pytest.skip("C++ compiler required for Quest 06")
    return compiler
...
def test_binary(tmp_path: Path) -> Path:
    ...
    binary = tmp_path / "ring_buffer_tests"
    command = [
        find_compiler(),
        "-std=c++20",
        "-pthread",
...
def test_fifo_and_full_state(test_binary: Path) -> None:
    run_case(test_binary, "fifo")


def test_wraparound_preserves_order(test_binary: Path) -> None:
    run_case(test_binary, "wrap")
...
def test_visualizer_shows_state(test_binary: Path) -> None:
    run_case(test_binary, "visualizer")
